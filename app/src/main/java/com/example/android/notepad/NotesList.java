...
public class NotesList extends ListActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // 如果 Intent 没有数据，则设置默认数据 URI
        if (intent.getData() == null) {
            intent.setData(NotePad.Notes.CONTENT_URI);
        }

        // 设置 ListView 的上下文菜单监听器
        getListView().setOnCreateContextMenuListener(this);

        // 获取数据库数据
        Cursor cursor = managedQuery(
                getIntent().getData(),            // 默认内容提供器的 URI
                PROJECTION,                       // 查询的列
                null,                             // 不需要条件
                null,                             // 没有查询参数
        ...
        searchBox.addTextChangedListener(new TextWatcher() {
            ...
            public void afterTextChanged(Editable s) {
                ...
                if (!query.isEmpty()) {
                    searchNotes(query, adapter); // 搜索笔记
                } else {
                    reloadNotes(adapter); // 显示所有笔记
                }
            }
     ...
     * 搜索笔记
     */
    private void searchNotes(String query, SimpleCursorAdapter adapter) {
        Uri searchUri = Uri.withAppendedPath(NotePad.Notes.CONTENT_URI, "search/" + query);
        ...
        adapter.swapCursor(searchCursor); // 更新适配器的游标
    }

    /**
     * 重新加载所有笔记
     */
    private void reloadNotes(SimpleCursorAdapter adapter) {
        ...
        adapter.swapCursor(cursor); // 更新适配器的游标
    }
